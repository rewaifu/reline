...
def cmyk_halftone(img: np.ndarray, dot_size: list[int], angle: list[int], dot_type: list[DotType]) -> np.ndarray:
    dot_size_len = len(dot_size)
    angle_len = len(angle)
    dot_type_len = len(dot_type)
    if img.ndim == 2:
...
def hsv_halftone(img: np.ndarray, dot_size: list[int], angle: list[int], dot_type: list[DotType]) -> np.ndarray:
    if img.ndim == 2:
...
def gray_halftone(img: np.ndarray, dot_size: list[int], angle: list[int], dot_type: list[DotType]) -> np.ndarray:
    if img.ndim == 3:
